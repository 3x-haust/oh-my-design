...
const REFERENCE_RESEARCH: InputSkeleton = {
  ...
  constraints: [
    ...
    'when ref discover-plan v2 returns marketReferencePolicy.mode=target-market-first, v7 marketCoverage is required. Execute its exact domainSearchInputs and market-and-domain-qualified design inputs before unqualified global searches. Each lane classifies every source id exactly once as localSources=[{sourceId,evidenceSha256,scope,basis,searchReceiptSha256}] or global fallback. The digest must bind the retained source image. basis=market-search-result requires the exact market-search receipt whose observed links contain the retained source/discovery URL; basis=market-domain requires a retained source/discovery host under the explicit market country-code domain and a null receipt. Every fallback gap records the exact marketRegion, availability|access|coverage kind, and exact attemptedQueries or attemptedRoots. Freeform prose cannot prove local provenance or a fallback attempt. A local URL is not a quality score or a country-style instruction.',
