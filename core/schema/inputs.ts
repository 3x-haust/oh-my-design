...
const REFERENCE_RESEARCH: InputSkeleton = {
  ...
  constraints: [
    ...
    'when ref discover-plan v2 returns marketReferencePolicy.mode=target-market-first, v7 marketCoverage is required. Execute its exact domainSearchInputs and market-and-domain-qualified design inputs before unqualified global searches. Each lane classifies every source id exactly once as localSources=[{sourceId,evidenceSha256,scope,basis,provenanceReceiptSha256}] or global fallback. The digest must bind the retained source image. basis=market-search-result requires the exact signed market-search receipt whose observed links contain the retained source/discovery URL. basis=market-direct-result requires the exact signed direct-public root receipt whose captured page visibly names the explicit market and service/product/gallery/audience scope and whose observed links contain the retained source/discovery URL. URL tokens, a country-code hostname, or freeform reason are not market-service proof. Every fallback gap records the exact marketRegion, availability|access|coverage kind, and exact attemptedQueries or attemptedRoots.',
