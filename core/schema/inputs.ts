...
const REFERENCE_RESEARCH: InputSkeleton = {
  ...
  constraints: [
    ...
    'when ref discover-plan v2 returns marketReferencePolicy.mode=target-market-first, v7 marketCoverage is required. Execute its exact domainSearchInputs and market-and-domain-qualified design inputs before unqualified global searches. Each lane classifies every source id exactly once as localSources=[{sourceId,evidenceSha256,reason}] or global fallback. The digest must bind the retained source image. Every local reason must name the explicit market and supported service, product, gallery, or audience scope; every fallback gap must name that market and a specific local availability, access, or coverage shortage. Generic quality prose is refused. A local URL is not a quality score or a country-style instruction.',
