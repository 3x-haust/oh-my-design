...
const REFERENCE_RESEARCH: InputSkeleton = {
  ...
  constraints: [
    ...
    'research-set publishes .omd/domain-references.json and .omd/design-references.json separately, then .omd/reference-research.json as a consistency receipt; research-check requires all three current records',
    'neither the same evidence path nor identical bytes under a renamed path can satisfy both lanes',
    'every design source needs discovery: an actual gallery/pin entry URL, kind (app-gallery, web-gallery, visual-bookmark, or an explicitly user-provided reference), access free, and a task-specific qualityReason; provider reputation alone is not quality evidence',
