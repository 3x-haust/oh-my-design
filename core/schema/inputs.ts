...
const REFERENCE_RESEARCH: InputSkeleton = {
  ...
  constraints: [
    ...
    'when ref discover-plan v2 returns marketReferencePolicy.mode=target-market-first, v7 marketCoverage is required. Execute its exact domainSearchInputs and market-and-domain-qualified design inputs before unqualified global searches. Each lane classifies every source id exactly once as localSources=[{sourceId,reason}] or global fallback, keeps at least one source-specific local-market reason, and gives a concrete gap for every nonempty fallback. A local URL is not a quality score or a country-style instruction.',
