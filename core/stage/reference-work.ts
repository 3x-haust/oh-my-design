...
export function referenceResearchWork(root: string, options: Readonly<{
  ...
  try { validateReferenceResearch(root, readPublishedReferenceResearch(root), options); }
    ...
    return {
      ...
      instruction: 'The board exists but current research is not published. This is Scout work inside reference-board, not a new route stage. Read schema reference-research and ref research-check --json; inspect both lanes and actual captures, repair the named input/evidence, then publish with ref research-set --input <research-input.json> --json. For a selected product benchmark use schema reference-flow-input and task-flow-benchmark, execute benchmark record and its publisher before binding its digest. Failed searches are gaps: use another public search or v6 direct-public discovery from ref discover-plan. Native ref navigate --entry public-directory (domain) or --entry free-gallery (design) publishes roots; follow observed links and capture retained entries separately. Never invent provenance or promote a directory into design evidence. Recompute stage next after research-check succeeds; do not author the application before its research inputs pass.',
    };
  }
