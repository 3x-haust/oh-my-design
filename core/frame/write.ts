 ...
 */
export function reframe(cwd: string, opts: { to: string; because: string }, adapter?: ProjectWriteAdapter): string {
  ...
  writeFrame(cwd, { ...frontmatter, revision, reframedAt: new Date().toISOString() }, appended, adapter);
  return join(designDir(cwd), 'frame.md');
}

export function setGenerator(cwd: string, generator: string, adapter?: ProjectWriteAdapter): void {
  const frame = readFrame(cwd);
  const { body = '', ...frontmatter } = frame ?? {};
  writeFrame(cwd, { ...frontmatter, generator }, body, adapter);
}

export function logDecision(cwd: string, what: string, why: string, adapter?: ProjectWriteAdapter): string {
  ...
  requireProjectWriteAdapter(cwd, adapter).write(
    '.omd/decisions.md',
...
export function logChoice(
  cwd: string,
  opts: { among: string[]; chose: string; why?: string },
  adapter?: ProjectWriteAdapter,
): string {
  ...
  requireProjectWriteAdapter(cwd, adapter).write('.omd/taste/preferences.jsonl', `${existing}${JSON.stringify(record)}\n`);
  return path;
}
