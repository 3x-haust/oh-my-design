...
export interface HoldoutRoute { surface: Exclude<Surface, 'mixed'>; }
export interface HoldoutBrief { id: string; kind: BriefKind; expectedDecision: ExpectedDecision; evidence: readonly string[]; surface: Surface; routes: readonly HoldoutRoute[]; domain: Domain; language: Language; critical?: boolean; deterministic?: boolean; }
export interface ProjectedBrief { id: string; kind: BriefKind; surface: Surface; routes: readonly HoldoutRoute[]; domain: Domain; language: Language; briefHash: string; evidenceAliases: readonly string[]; }
export interface EvaluatorHoldout { id: string; kind: BriefKind; surface: Surface; routes: readonly HoldoutRoute[]; domain: Domain; language: Language; expectedDecision: ExpectedDecision; critical: boolean; deterministic: boolean; }
...
export type LegalCell = Readonly<{ surface: Surface; kind: BriefKind; expectedDecision: ExpectedDecision; domain: Domain; language: Language }>;
const legalSurfaceRegisterMotion: readonly Pick<LegalCell, 'surface' | 'kind' | 'expectedDecision'>[] = Object.freeze([
  { surface: 'marketing', kind: 'silent-evidence', expectedDecision: 'one' },
  ...
  { surface: 'mixed', kind: 'quiet', expectedDecision: 'none' },
]);
export function canonicalLegalCellManifest(): readonly LegalCell[] { return Object.freeze(legalDomains.flatMap(domain => legalLanguages.flatMap(language => legalSurfaceRegisterMotion.map(cell => Object.freeze({ ...cell, domain, language }))))); }

export function createEvaluatorHoldout(brief: HoldoutBrief): EvaluatorHoldout { return { id: brief.id, kind: brief.kind, surface: brief.surface, routes: Object.freeze(brief.routes.map(route => Object.freeze({ ...route }))), domain: brief.domain, language: brief.language, expectedDecision: brief.expectedDecision, critical: brief.critical === true, deterministic: brief.deterministic === true }; }
...
export function projectHoldoutBrief(brief: HoldoutBrief, salt: string): ProjectedBrief {
  ...
  return Object.freeze({ id, kind: brief.kind, surface: brief.surface, routes: Object.freeze(brief.routes.map(route => Object.freeze({ ...route }))), domain: brief.domain, language: brief.language, briefHash: sha256(canonical({ id, kind: brief.kind, surface: brief.surface, routes: brief.routes, domain: brief.domain, language: brief.language, salt })), evidenceAliases: Object.freeze(brief.evidence.map((evidence, index) => `evidence-${index + 1}-${sha256(`${salt}:${evidence}`).slice(0, 16)}`)) });
}
export function validateProjectedBrief(projected: ProjectedBrief): void {
  ...
  if (!/^holdout-[a-f0-9]{32}$/.test(projected.id) || !hashPattern.test(projected.briefHash) || !legalSurfaces.includes(projected.surface) || !legalDomains.includes(projected.domain) || !legalLanguages.includes(projected.language) || !projected.routes.length || projected.routes.some(route => !route || !legalSurfaces.includes(route.surface)) || !projected.evidenceAliases.length || projected.evidenceAliases.some(alias => !/^evidence-\d+-[a-f0-9]{16}$/.test(alias))) throw new Error('projected brief has invalid fields');
}
...
function cellKey(brief: Pick<LegalCell, 'surface' | 'kind' | 'expectedDecision' | 'domain' | 'language'>): string { return `${brief.surface}:${brief.kind}:${brief.expectedDecision}:${brief.domain}:${brief.language}`; }
function hasLegalRoute(surface: Exclude<Surface, 'mixed'>, kind: BriefKind, expectedDecision: ExpectedDecision): boolean { return legalSurfaceRegisterMotion.some(cell => cell.surface === surface && cell.kind === kind && cell.expectedDecision === expectedDecision); }
function validateRoutes(brief: Pick<HoldoutBrief, 'id' | 'surface' | 'routes' | 'kind' | 'expectedDecision'>): void {
  const routes = brief.routes;
  if (!Array.isArray(routes) || !routes.length || routes.some(route => !route || route.surface === 'mixed' || !legalSurfaces.includes(route.surface)) || new Set(routes.map(route => route.surface)).size !== routes.length) throw new Error(`invalid route authority: ${brief.id}`);
  if (brief.surface === 'mixed') {
    if (routes.length < 2 || !routes.every(route => hasLegalRoute(route.surface, brief.kind, brief.expectedDecision))) throw new Error(`mixed routes are not individually legal: ${brief.id}`);
  } else if (routes.length !== 1 || routes[0]!.surface !== brief.surface) throw new Error(`route authority does not match surface: ${brief.id}`);
}
function validateLegalCells(briefs: readonly Pick<LegalCell & { id: string; routes: readonly HoldoutRoute[] }, 'id' | 'surface' | 'kind' | 'expectedDecision' | 'domain' | 'language' | 'routes'>[], label: string): void {
  const ids = new Set<string>(); const cells = new Set<string>();
  for (const brief of briefs) {
    if (!brief.id || ids.has(brief.id) || !legalSurfaces.includes(brief.surface) || !legalDomains.includes(brief.domain) || !legalLanguages.includes(brief.language) || !legalDecisions.includes(brief.expectedDecision) || !legalSurfaceRegisterMotion.some(cell => cell.surface === brief.surface && cell.kind === brief.kind && cell.expectedDecision === brief.expectedDecision)) throw new Error(`invalid legal-cell brief: ${brief.id}`);
    validateRoutes(brief); ids.add(brief.id); const cell = cellKey(brief);
    if (cells.has(cell)) throw new Error(`duplicate legal cell: ${cell}`); cells.add(cell);
  }
  const manifest = canonicalLegalCellManifest();
  if (cells.size !== manifest.length || manifest.some(cell => !cells.has(cellKey(cell)))) throw new Error(`${label} is missing, extra, or illegal cells`);
}
export function validateLegalSurfaceMatrix(briefs: readonly HoldoutBrief[], routeMap: Readonly<Record<string, Surface>>): void {
  if (Object.keys(routeMap).length !== briefs.length || briefs.some(brief => !Array.isArray(brief.evidence) || !brief.evidence.length || routeMap[brief.id] !== brief.surface)) throw new Error('invalid legal-cell route or evidence authority');
  validateLegalCells(briefs, 'canonical legal-cell manifest');
}
export function validateDevelopmentCorpus(briefs: readonly HoldoutBrief[], routeMap: Readonly<Record<string, Surface>> = Object.fromEntries(briefs.map(brief => [brief.id, brief.surface]))): void { validateLegalSurfaceMatrix(briefs, routeMap); }
export function freezeDevelopmentCorpus(briefs: readonly HoldoutBrief[], routeMap: Readonly<Record<string, Surface>>): FrozenDevelopmentCorpus { validateDevelopmentCorpus(briefs, routeMap); const snapshot = briefs.map(brief => Object.freeze({ ...brief, routes: Object.freeze(brief.routes.map(route => Object.freeze({ ...route }))), evidence: Object.freeze([...brief.evidence]) })); const frozenRouteMap = Object.freeze({ ...routeMap }); return Object.freeze({ briefs: Object.freeze(snapshot), routeMap: frozenRouteMap, digest: hashRecord({ briefs: snapshot, routeMap: frozenRouteMap }) }); }
export function validateFrozenDevelopmentCorpus(corpus: FrozenDevelopmentCorpus): void { if (!Object.isFrozen(corpus) || !Object.isFrozen(corpus.briefs) || !Object.isFrozen(corpus.routeMap) || corpus.briefs.some(brief => !Object.isFrozen(brief) || !Object.isFrozen(brief.routes) || brief.routes.some(route => !Object.isFrozen(route)) || !Object.isFrozen(brief.evidence)) || !hashPattern.test(corpus.digest) || corpus.digest !== hashRecord({ briefs: corpus.briefs, routeMap: corpus.routeMap })) throw new Error('development corpus is not frozen or its digest drifted'); validateDevelopmentCorpus(corpus.briefs, corpus.routeMap); }
export function validateEvaluatorHoldoutMetadata(metadata: EvaluatorHoldoutMetadata): void { if (!Object.isFrozen(metadata) || !Object.isFrozen(metadata.holdouts) || metadata.holdouts.some(holdout => !Object.isFrozen(holdout) || !Object.isFrozen(holdout.routes) || holdout.routes.some(route => !Object.isFrozen(route))) || !hashPattern.test(metadata.digest) || metadata.digest !== hashRecord(metadata.holdouts)) throw new Error('evaluator holdout metadata is not frozen or its digest drifted'); validateLegalCells(metadata.holdouts, 'evaluator holdout'); const silentMarketingOne = metadata.holdouts.filter(holdout => holdout.surface === 'marketing' && holdout.kind === 'silent-evidence' && holdout.expectedDecision === 'one').length; const quietNone = metadata.holdouts.filter(holdout => holdout.kind === 'quiet' && holdout.expectedDecision === 'none').length; if (silentMarketingOne < 2 || quietNone < 2 || !metadata.holdouts.some(holdout => holdout.kind === 'showpiece' && holdout.expectedDecision === 'one')) throw new Error('evaluator holdout lacks required lawful motion coverage'); }
