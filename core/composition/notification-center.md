...
A product-surface pattern for reviewing durable events that may require later action. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is reviewing durable events that may require later action. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for brief success feedback that belongs in a toast. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Unread summary, filters, chronological feed, notification detail. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
Unread state is structural, not a colored dot alone. Notifications keep source, time, consequence, and next action together.
...
Use a landmark feed or list. Each item has a heading and time element. Mark-read controls expose their state and do not hijack row activation.
...
Tab reaches item actions, not every static row. Enter opens a linked event. M marks read only when the shortcut is documented and focus is in the feed. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `알림 8개, 읽지 않음 3개` over an abstract system label. For zero results, say `새 알림이 없어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">알림 8개, 읽지 않음 3개</h1>
      <p class="recipe__status" role="status">새 알림이 없어요.</p>
    </div>
    <button class="primary" type="button">Notification 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="알림 8개, 읽지 않음 3개">
    <!-- Bind real notification content and reachable state regions here. -->
  </section>
</main>
```
