...
A product-surface pattern for deciding what needs attention now across a small set of operational signals. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is deciding what needs attention now across a small set of operational signals. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for celebrating vanity metrics or displaying unrelated KPI tiles. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Status summary, prioritized work queue, exceptions, recent changes. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
The queue is the anchor. Totals appear beside the records they summarize. Exceptions and stale work outrank decorative trends.
...
Use headings, a compact summary list, and a real table or list for actionable work. Charts appear only when a trend changes the next decision.
...
Tab reaches filters and row actions. Enter opens a record. Shortcuts are documented and never replace visible controls. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `오늘 처리할 작업 14건` over an abstract system label. For zero results, say `지금 처리할 작업이 없어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">오늘 처리할 작업 14건</h1>
      <p class="recipe__status" role="status">지금 처리할 작업이 없어요.</p>
    </div>
    <button class="primary" type="button">Work Item 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="오늘 처리할 작업 14건">
    <!-- Bind real work item content and reachable state regions here. -->
  </section>
</main>
```
