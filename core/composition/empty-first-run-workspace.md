...
A product-surface pattern for a usable product has no user-created objects yet and needs one clear first step. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is a usable product has no user-created objects yet and needs one clear first step. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for filtered zero results, permission failures, or a product that can show useful sample data. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Orientation, first-value action, concise example, optional import path. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
The empty work region is the front door. It explains what will exist here, offers one primary creation path, and keeps import secondary.
...
The default row height is `56px`. Controls are 40px on desktop and 44px on touch layouts. Use 8px within a control, 12px between related controls, 16px between rows or subgroups, 24px between regions, and 32px at page edges. Compact density is earned only by frequent expert scanning.

Use a heading, brief paragraph, and one primary button inside the normal work region. Do not fake cards, charts, or disabled sample controls.
...
Tab reaches create, then import and help. Enter activates the focused path. Returning from creation restores focus and shows the new object. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `첫 프로젝트를 만들어 보세요.` over an abstract system label. For zero results, say `아직 만든 프로젝트가 없어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">첫 프로젝트를 만들어 보세요.</h1>
      <p class="recipe__status" role="status">아직 만든 프로젝트가 없어요.</p>
    </div>
    <button class="primary" type="button">Project 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="첫 프로젝트를 만들어 보세요.">
    <!-- Bind real project content and reachable state regions here. -->
  </section>
</main>
```
