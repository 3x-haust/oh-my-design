...
A product-surface pattern for giving expert users a fast, searchable path to known actions and destinations. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is giving expert users a fast, searchable path to known actions and destinations. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for primary navigation, first-run discovery, or commands that need complex input. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Trigger, dialog search, grouped results, command outcome. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
The palette overlays the current context but does not replace visible navigation. Results state scope, shortcut, and disabled reason.
...
Use a modal dialog with a combobox and listbox relationship. Keep commands as real buttons or links and restore focus to the opener.
...
Cmd or Ctrl plus K opens. Up and Down move the active option. Enter runs it. Escape closes. Tab remains inside while open. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `명령어 검색` over an abstract system label. For zero results, say `일치하는 명령어가 없어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">명령어 검색</h1>
      <p class="recipe__status" role="status">일치하는 명령어가 없어요.</p>
    </div>
    <button class="primary" type="button">Command 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="명령어 검색">
    <!-- Bind real command content and reachable state regions here. -->
  </section>
</main>
```
