...
A product-surface pattern for changing durable preferences grouped by user intent. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is changing durable preferences grouped by user intent. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for a short one-off form or operational work queue. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Intent navigation, one-column groups, save feedback, danger zone. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
Groups follow user goals, not database tables. Each control says whether it saves immediately or with the single Save action.
...
Use fieldsets and legends for related options. Switches change state immediately; checkboxes join a later submit. Place destructive account actions last.
...
Tab follows the single column. Space toggles a checkbox or switch. Escape never silently discards edits. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `설정` over an abstract system label. For zero results, say `저장할 수 있는 설정이 없어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      ...
      <p class="recipe__status" role="status">저장할 수 있는 설정이 없어요.</p>
    </div>
    <button class="primary" type="button">Setting 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="설정">
    <!-- Bind real setting content and reachable state regions here. -->
  </section>
</main>
```
