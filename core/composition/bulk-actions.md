...
A product-surface pattern for applying the same operation to several visible records. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is applying the same operation to several visible records. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for single-record work or operations whose consequences differ per record. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Selectable collection, selection summary, bulk toolbar, outcome report. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
Selection is explicit and scoped. The toolbar appears next to the collection without moving its header. Select all distinguishes this page from all matching results.
...
Use one checkbox per row and a labeled select-all checkbox. Announce selection count in a polite live region. Confirm destructive batches with count and scope.
...
Space toggles the focused row checkbox. Shift plus click may extend pointer selection, but keyboard users get select-page and clear controls. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `12개 선택됨` over an abstract system label. For zero results, say `선택할 항목이 없어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">12개 선택됨</h1>
      <p class="recipe__status" role="status">선택할 항목이 없어요.</p>
    </div>
    <button class="primary" type="button">Item 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="12개 선택됨">
    <!-- Bind real item content and reachable state regions here. -->
  </section>
</main>
```
