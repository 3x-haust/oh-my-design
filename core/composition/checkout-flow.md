...
A product-surface pattern for completing a purchase with address, delivery, and payment decisions. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is completing a purchase with address, delivery, and payment decisions. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for browsing products or changing account settings. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Order summary, named progress, form step, review, confirmation. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
The order summary and total remain available at every step. Guest checkout is offered. Fields are limited to decisions needed for fulfillment.
...
Use one form column, native autocomplete tokens, field errors adjacent to inputs, and a final review before the payment commitment.
...
Tab follows task order. Enter submits only when it cannot trigger an accidental purchase. Back preserves values and Escape does not erase the cart. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `결제하기` over an abstract system label. For zero results, say `주문할 상품이 없어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">결제하기</h1>
      <p class="recipe__status" role="status">주문할 상품이 없어요.</p>
    </div>
    <button class="primary" type="button">Order 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="결제하기">
    <!-- Bind real order content and reachable state regions here. -->
  </section>
</main>
```
