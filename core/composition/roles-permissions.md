...
A product-surface pattern for assigning capabilities across roles where mistakes have security consequences. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is assigning capabilities across roles where mistakes have security consequences. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for simple member invitations with one fixed access level. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Role list, permission matrix, affected-member summary, audit note. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
Rows are capabilities grouped by domain; columns are roles only when simultaneous comparison matters. Effective access is distinguished from inherited access.
...
The default row height is `44px`. Controls are 40px on desktop and 44px on touch layouts. Use 8px within a control, 12px between related controls, 16px between rows or subgroups, 24px between regions, and 32px at page edges. Compact density is earned only by frequent expert scanning.

Use a table for a true matrix, checkboxes with full labels, and `aria-describedby` for consequences. Never use color alone for inherited or denied states.
...
Tab reaches each permission. Space toggles it. Shift plus Space is not overloaded. A change summary receives focus before confirmation. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `권한 변경 사항 3개` over an abstract system label. For zero results, say `변경할 수 있는 권한이 없어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">권한 변경 사항 3개</h1>
      <p class="recipe__status" role="status">변경할 수 있는 권한이 없어요.</p>
    </div>
    <button class="primary" type="button">Permission 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="권한 변경 사항 3개">
    <!-- Bind real permission content and reachable state regions here. -->
  </section>
</main>
```
