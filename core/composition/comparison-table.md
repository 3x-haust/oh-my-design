...
A product-surface pattern for comparing the same attributes across a small set of alternatives. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is comparing the same attributes across a small set of alternatives. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for items with different attributes or a persuasive featured-plan card row. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Attribute groups, alternatives, aligned values, decision action. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
Attributes own rows and alternatives own columns. Differences can be highlighted, but no option is enlarged without task-backed priority.
...
Use table headers with correct scope. Repeat the alternative name in narrow staged views. Explain symbols in text, not tooltip-only content.
...
Tab skips static cells and reaches controls. Arrow navigation is optional only for a true interactive grid with full grid semantics. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `요금제 비교` over an abstract system label. For zero results, say `비교할 항목을 추가하세요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">요금제 비교</h1>
      <p class="recipe__status" role="status">비교할 항목을 추가하세요.</p>
    </div>
    <button class="primary" type="button">Option 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="요금제 비교">
    <!-- Bind real option content and reachable state regions here. -->
  </section>
</main>
```
