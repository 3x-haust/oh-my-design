...
A product-surface pattern for choosing a product variant and understanding evidence needed to buy. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is choosing a product variant and understanding evidence needed to buy. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for an operational inventory record or a product with no meaningful variants. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Media, product facts, variant choice, purchase action, delivery and returns. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
Price, availability, variant, and purchase action form one decision region. Evidence follows the questions that block purchase, not a marketing section formula.
...
Use radio buttons for mutually exclusive variants and a button for add-to-cart. Images have useful alt text or empty alt when decorative.
...
Tab reaches choices and actions. Arrow keys move within radio groups. Changing a variant updates price and availability with a live status. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `옵션을 선택하세요` over an abstract system label. For zero results, say `현재 구매할 수 없는 상품이에요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">옵션을 선택하세요</h1>
      <p class="recipe__status" role="status">현재 구매할 수 없는 상품이에요.</p>
    </div>
    <button class="primary" type="button">Product 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="옵션을 선택하세요">
    <!-- Bind real product content and reachable state regions here. -->
  </section>
</main>
```
