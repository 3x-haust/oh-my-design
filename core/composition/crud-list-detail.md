...
A product-surface pattern for creating, scanning, editing, and deleting the same object type repeatedly. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is creating, scanning, editing, and deleting the same object type repeatedly. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for a read-only collection or an immersive editor that needs the whole viewport. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Collection toolbar, record list, selected detail, edit actions. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
The list keeps identity and status visible while detail work happens. Creation is one primary action. Destruction is separated from save.
...
The default row height is `48px`. Controls are 40px on desktop and 44px on touch layouts. Use 8px within a control, 12px between related controls, 16px between rows or subgroups, 24px between regions, and 32px at page edges. Compact density is earned only by frequent expert scanning.

Use links or buttons for row opening, a labeled detail region, native form controls, and a danger confirmation only for irreversible deletion.
...
Up and Down may move an explicitly implemented listbox selection. Enter opens detail. Escape closes a temporary drawer and returns focus. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `고객 18명` over an abstract system label. For zero results, say `조건에 맞는 고객이 없어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">고객 18명</h1>
      <p class="recipe__status" role="status">조건에 맞는 고객이 없어요.</p>
    </div>
    <button class="primary" type="button">Customer 추가</button>
  </header>
  <section class="recipe__toolbar" aria-label="찾기 및 범위">
    <label>검색 <input type="search" name="q"></label>
    <button type="button">필터</button>
  </section>
  <section class="recipe__work" aria-label="고객 18명">
    <!-- Bind real customer content and reachable state regions here. -->
  </section>
</main>
```
