...
A product-surface pattern for letting users reach first value through optional tasks in their own order. The work object, not decorative framing, owns the first viewport. See `core/theory/ux.md`, `core/theory/layout.md`, and `core/theory/components.md`.

## When it earns its place / When it does not

Use it when the primary task is letting users reach first value through optional tasks in their own order. It should shorten the repeated loop from orientation to action to visible feedback.

Do not use it for mandatory dependent decisions that require a wizard. If the task can be completed with a simpler list, form, or direct navigation, choose that cheaper structure.

## Structure and grid

**1280 desktop:** use a 12-column content grid inside 32px page margins, with a 24px gutter. The sequence is Value statement, progress, task list, skip or dismiss, completion state. The dominant work region spans the available width; supporting context may use up to four columns only when concurrent visibility changes the decision.
...
Tasks are real outcomes, not feature tours. The shortest path to value comes first, and completed work stays visible without dominating.
...
Use an ordered or unordered list with buttons or links. Progress has a text equivalent. Each task states result and expected effort.
...
Tab reaches incomplete task actions. Enter opens one. Completed tasks remain reviewable. Escape closes optional guidance without resetting progress. Focus remains visible, and any temporary surface restores focus to its trigger. Never attach row activation to a nonfocusable container.

## Korean copy notes

Use the user's nouns and short verb endings. Prefer `시작하기 2/4 완료` over an abstract system label. For zero results, say `지금 바로 시작할 수 있어요.` and name the recovery action nearby. Avoid English uppercase eyebrow labels, forced letter spacing, and noun stacks copied from database fields. Let Hangul wrap naturally; inspect 390px and 320px for clipping and one-syllable orphans. Counts, won amounts, dates, and status terms must stay unambiguous.
...
<main class="recipe" aria-labelledby="page-title">
  <header class="recipe__header">
    <div>
      <h1 id="page-title">시작하기 2/4 완료</h1>
      <p class="recipe__status" role="status">지금 바로 시작할 수 있어요.</p>
    </div>
  ...
  <section class="recipe__work" aria-label="시작하기 2/4 완료">
