...
- Stack conformance is a ship gate: run `omd stack --check` once production source exists. It fails when a greenfield materialised as plain HTML/CSS/JS with no package.json and no recorded verbatim user request for HTML — a stack-routing defect. The build must be React + Vite + TypeScript (`omd stack`), not vanilla HTML the model talked itself into. `omd check` renders an HTML page and a React page identically and cannot see this, so this deterministic check is the only thing that catches the drift.
