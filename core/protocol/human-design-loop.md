...
- Register ambition is part of the GREEN target: a `marketing` or showpiece surface must carry exactly one committed signature moment — concept-bearing motion plus a layout that departs from the generic template — reached by default even from a rough brief. The eye owns this verdict (`agents/eye.agent.yaml`): a merely functional element (a working copy button, a form, a nav, a terminal that only runs a command) is baseline function, never the signature moment, so a clean surface whose only "carrier" is something it needed to work anyway is RED. A safe, evenly-spaced, inoffensive result any generator would produce — a centered hero over uniformly stacked single-column sections — is RED for that register, not a neutral pass (`theory/expressive.md` § "The register decision"). The eye must be able to name the generic template and point to the one rendered departure that breaks it (asymmetry, a large scale/weight contrast, a grid break, a section inversion, or real material made structural). The restraint clause still caps it at one bold concept, not a catalogue; usability (reduced-motion, keyboard, performance budget) still holds; and it never travels down-register — a `product`/quiet surface stays quiet, where ambition is density, scanning, and fewer errors.
