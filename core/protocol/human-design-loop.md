...
- Stack conformance is an early gate and a ship gate: run `omd stack --check` right after the greenfield scaffold (before authoring any surface) and again before ship — not only at the end. It fails when the folder shows plain HTML/CSS/JS with no package.json and no recorded verbatim user request for HTML, so a hand that wrote `index.html` as its first file instead of scaffolding React + Vite + TypeScript is caught at that first write, before the whole build is wasted. `omd check` renders an HTML page and a React page identically and cannot see this; this deterministic check is the only thing that catches the drift.
