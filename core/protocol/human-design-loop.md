...
- Register ambition is part of the GREEN target: a `marketing` or showpiece surface must carry exactly one committed signature moment — concept-bearing motion plus a layout that departs from the generic template — reached by default even from a rough brief. A safe, evenly-spaced, inoffensive result any generator would produce is RED for that register, not a neutral pass (`theory/expressive.md` § "The register decision"). The restraint clause still caps it at one bold concept, not a catalogue; usability (reduced-motion, keyboard, performance budget) still holds; and it never travels down-register — a `product`/quiet surface stays quiet, where ambition is density, scanning, and fewer errors.
