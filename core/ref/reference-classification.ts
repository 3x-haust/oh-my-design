...
const MARKER = /^(content-only|anti-reference):\s+(.+)$/;
...
export function parseReferenceClassification(value: Readonly<{ principles: unknown }>): ReferenceClassification {
  ...
  const marked: { kind: 'content-only' | 'anti-reference'; statement: string }[] = [];
  for (const principle of value.principles) {
    ...
    const kind = match[1] as 'content-only' | 'anti-reference';
