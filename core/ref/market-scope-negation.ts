export function negatesMarketScope(value: string, tokens: readonly string[]): boolean {
  ...
  return tokens.some(candidate => {
    ...
    while (index >= 0) {
      ...
      const positiveBefore = /no\s+service\s+is\s+available\s+outside\s*$|not\s+unavailable\s+in\s*$/u.test(before);
      const positiveAfter = /^\s*(?:is|was)\s+excluded\s+from\s+(?:the\s+)?unsupported\s+list\b/u.test(after);
      const negatedBefore = /(?:is\s+not|isn't|not)\s+available\s+in\s*$|(?:does\s+not|doesn't)\s+(?:serve|support)(?:\s+[\p{L}\p{N}-]+){0,4}\s+in\s*$|not\s+(?:serving|supporting)\s*$|unavailable\s+to(?:\s+[\p{L}\p{N}-]+){0,4}\s+(?:in|of)\s*$|(?:unsupported|unavailable)\s+in\s*$|(?:outside|excludes?|excluding)\s*$/u.test(before)
      ...
      const negatedAfter = /^\s*(?:은|는|이|가|을|를|에서는|거주자는)?.{0,32}(?:이용할\s*수\s*없|지원\s*대상이\s*(?:아니|아닙)|미지원|제외|불가|지원하지\s*않|제공하지\s*않)/u.test(after)
        || /^\s*(?:residents?|users?)?\s*(?:(?:is|are|was|were)\s+)?(?:not\s+served|not\s+supported|not\s+available|unavailable|unsupported|excluded)\b/u.test(after)
