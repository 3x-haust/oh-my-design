...
type SearchExecution = Readonly<{
  ...
  observedAt: string; status: 'page-observed' | 'blocked' | 'http-error' | 'navigation-error'; httpStatus: number | null;
...
export function parseSearchInput(value: unknown): SearchInput {
  ...
  if (input.queryParam !== 'q' || !SEARCH_ENDPOINTS.has(`${parsed.origin}${parsed.pathname}`)) return fail('unsupported search endpoint; use a public Google, Bing or DuckDuckGo search URL with q');
...
function parseReceipt(value: unknown, lane: Lane, extension: string): Receipt {
  ...
  if (path !== `.omd/refs/${lane}/search-${sha256}.${extension}`) return fail('search receipt must use its exact content-addressed lane path');
  return { path, sha256 };
}
function parseExecution(value: unknown): SearchExecution {
  ...
  if (row.schema !== SEARCH_EXECUTION_SCHEMA || row.limitations !== LIMITATIONS || row.provider !== new URL(input.url).hostname
    || !['page-observed', 'blocked', 'http-error', 'navigation-error'].includes(row.status as string)
  ...
  if (row.status === 'page-observed') parseSearchInput({ ...input, url: row.finalUrl });
  if (row.status !== 'page-observed' && row.error === null) return fail('failed attempt needs a reason');
...
export async function executeReferenceSearch(browser: Browser, value: unknown, writer: ProjectWriteAdapter): Promise<Receipt> {
  ...
  try {
    ...
    capture = { path: `.omd/refs/${input.lane}/search-${hash(bytes)}.png`, sha256: hash(bytes) };
    ...
    status = challenge !== null ? 'blocked' : httpStatus === 200 ? 'page-observed' : 'http-error';
    ...
    if (status === 'page-observed') {
      try { parseSearchInput({ ...input, url: finalUrl }); }
  ...
  const receipt = { path: `.omd/refs/${input.lane}/search-${hash(bytes)}.json`, sha256: hash(bytes) };
