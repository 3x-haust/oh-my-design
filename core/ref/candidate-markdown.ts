...
export function formatReferenceCandidates(raw: RawReferenceBoard, assembly: ReferenceAssembly): string {
  ...
  const sections = assembly.candidates.map((candidate) => {
    ...
    const rows = candidate.pieces.map((piece) => {
      ...
      return `| ${cell(source(rawPieceValue))} | ${cell(part(rawPieceValue))} | ${cell(`${candidate.route} → ${piece.targetComponent}`)} | ${cell(piece.take.join(', '))} | ${cell(piece.avoid)} | ${cell(piece.adaptation)} |`;
    });
    return [`## 후보 (Candidate): ${cell(candidate.label)}`, `경로 (Route): ${cell(candidate.route)}`, `근거 (Rationale): ${cell(candidate.rationale)}`, '', '| 출처 사이트/페이지 (Source site/page) | 정확한 UI/이미지 부분 (Exact UI/image part) | 제안 대상 경로/컴포넌트 (Proposed target route/component) | 가져올 점 (Take) | 피할 점 (Avoid) | 적용 방식 (Adaptation) |', '| --- | --- | --- | --- | --- | --- |', ...rows].join('\n');
  });
