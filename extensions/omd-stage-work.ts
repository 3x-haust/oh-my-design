...
export class StageWork {
  ...
  activate(cwd: string, prompt: string): void {
    ...
    this.tasks.set(cwd, { token: Symbol(), checked: new Set(), pending: new Map(), started: false });
  }
  token(cwd: string): symbol | undefined { return this.tasks.get(cwd)?.token; }
  started(cwd: string): boolean { return this.tasks.get(cwd)?.started === true; }
  ...
  commandSucceeded(cwd: string, command: Readonly<{ args: readonly string[]; token: symbol | undefined }>): void {
    const task = this.tasks.get(cwd);
    if (!task || task.token !== command.token) return;
    const { args } = command;
    ...
    if (stage !== undefined && task.checked.has(stage)) task.started = true;
  }
