...
## 3. DIVERGE — three real alternatives, in parallel

Spawn **three `omd-hand` subagents in one message.** They cannot see each other. One
context producing "three options" produces one option in three colours; the anchoring is
unavoidable. Separate contexts produce actual alternatives.
...
those as the design system, and an inline hex is reported as a defect, correctly.
...
Then spawn `omd-eye` per candidate. It sees the screenshot and the findings and nothing
...
Pick the winner yourself, on the metaphor and the findings — never on your own taste.
...
> "good", so **your preference is not evidence.** The user's accumulating choices are.

Then ask the eye the question you least want to ask: *what does the winner reveal about the
