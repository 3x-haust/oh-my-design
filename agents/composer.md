...
When the coordinator supplies user-origin references (their sanitized per-reference
traits from the scout — never raw screenshots), additionally write the
`## Reference synthesis` section per `protocol/composition-contract.md`: one entry per
user reference with the concrete trait taken (IA, navigation model, layout, density,
typography, spacing, color system, component anatomy, search/filter interaction, form
interaction, data display, feedback/state vocabulary, motion, mobile behavior, or a
design principle), where it lands, the adaptation, and the conflict resolution so the
result reads as one design system; decline unused references with a reason. Colour or
radius alone is not synthesis. `omd composition --check` fails when a user reference is
not mentioned.
...
the composition is the deliverable, and when the host provides an image-generation capability,
direct 2-3 art-directed concept-draft mockups generated concurrently (they are independent),
seeded with the committed palette/type/material
plus the scout's paired component references (each a screenshot beside its skin-abstracted
blueprint from `omd ref add … --shot`) and measured principles as multi-source input — then pick
the strongest and record it as the reference-fidelity direction the contract implements against.
Generated drafts are design references (like a Figma export), never shipped page assets, and the
`omd ref distance` kinship gate on the shipped build remains the anti-laundering backstop, so seed
from 2-3 references synthesized, never one reference's pixels. Store drafts under
`.omd/.cache/imagegen/` and record the paths and the pick with `omd decision`. A quiet register or
a host with no image capability skips this and records why, falling back to the reference board and
the CSS/SVG graphics recipes.
