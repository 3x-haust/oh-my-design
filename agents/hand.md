...
the selected assembly plus the CSS/SVG evidence path. Open the selected reference's local part-image
capture(s) under `.omd/refs/` (resolved through `.omd/reference-selection.json`) and build against
them with image-to-code fidelity — copying the reference's layout, composition, spacing, and
treatment is allowed and encouraged, not just for a user-directed reference. `omd ref distance`
measures how close your build is to each chosen reference; high closeness is the intended outcome.
Record attribution for every used reference in `.omd/attribution.md`, and write the product's own
copy rather than lifting the source's words. Run
...
Signature-moment floor and ceiling by register: a `marketing` or showpiece surface must carry
exactly one signature moment (concept-bearing motion + a template-breaking layout) — required by
default even from a rough brief, and a safe, generic, inoffensive build is a miss for it; a quiet
or `product` surface carries none. Never a catalogue of techniques — one bold concept committed
fully, never eight competing.
...
write `.omd/final-evidence.json`. When reference assembly applies, record every selected slot's actual
`used`, `rejected`, or `anti-reference` outcome during this real build with
`recordReferenceUsage(root, { rows })`, including its exact target, borrowed and explicitly
non-borrowed properties, transformation, production evidence, and verification note. Run
`validateReferenceUsage(root)` before handing the ledger to the finalizer. Never write the final
report yourself or claim influence without a validated `used` row. Any source or build mutation requires the protocol's reseal,
