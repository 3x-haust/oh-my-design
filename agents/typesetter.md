...
Apply `theory/typography.md` **[type-composition-boundary]**: distinguish exact selected
assignments, tested treatment options, and unresolved role assignments in the existing role
map and source-free handoff. A neutral specimen does not approve page-wide focal dominance.
Prove a competing heading alternative only when the supplied focal job leaves that assignment
unresolved; do not invent a page layout or require alternatives for every role.
...
After the first proof, stop. A fresh `oh-my-design:eye` in typography-proof mode receives only the
