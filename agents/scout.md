...
`--from-user`. Author a board or candidate assemblies only when the adaptive strategy selects that
method; otherwise hand measured records directly to the next selected stage. There is no reference
quota and no default candidate count.
...
Capture at desktop width. `omd ref add` and `omd ref add-batch` now default to 1440x900 and
measure motion energy on every capture; do not pass a phone viewport or `--no-energy` for a zone
that composes a desktop layout. A board measured at phone width holds column fragments — a title
...
Match stable fallbacks by the framer-owned zone's semantics, not by one literal zone ID. Inspect
both `zones[].id` and `zones[].job` before writing the initial manifest:
- A required preference-control zone (language, locale, theme, appearance, or preferences) uses
  `https://designsystem.digital.gov/components/header/` with selector `.usa-header` in the initial
  batch. Bind it to the exact framer-owned zone ID. Do not spend either attempt guessing hashed
  classes on Vite, Mintlify, or another frequently changing marketing header.
- A required first-message/hero/adoption-message zone uses
  `https://designsystem.digital.gov/templates/landing-page/` with selector `.site-page-title` in
  the initial batch. Treat this deliberately narrow, low-signal capture only as title hierarchy;
  do not claim that it supplies a whole hero or visual identity. Never request the nonexistent
  `designsystem.digital.gov/components/hero/` page or guess `.usa-hero` on the landing template.
- A required CTA/install/copy/action zone other than the repository-specific case below uses
  `https://designsystem.digital.gov/components/button/` with selector
  `.site-component-preview .usa-button` in the initial batch. This tested selector scopes the
  visible component preview; do not use the page's first generic `.usa-button` or guess
  `.usa-button--big`.
...
batch uses `https://designsystem.digital.gov/components/button/` with selector
`.site-component-preview .usa-button`, bound to the same exact zone ID. The fallback transfers
accessible outbound-action anatomy while subject identity remains grounded in the user-supplied
repository facts.
