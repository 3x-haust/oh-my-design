...
keep at least one target-market source in each lane with a source-specific reason that names the
explicit market, explains which service, product, gallery, or audience scope it supports, and binds
the retained image's exact SHA-256. A fallback gap must name the same market and its specific local
availability, access, or coverage shortage; generic quality prose is refused. `research-check`
