...
`searchReceiptSha256`. `market-search-result` must bind the exact market-search receipt whose observed
links contain the retained source or design-discovery URL. `market-domain` must use a null receipt and
a retained source/discovery host under the explicit market country-code domain. A fallback gap records
