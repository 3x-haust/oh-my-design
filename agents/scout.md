...
observed links contain the retained source or design-discovery URL. `market-direct-result` must bind the
...
design-discovery URL. URL tokens, a country-code hostname, or freeform reason prove neither. A fallback gap records
the exact market, an availability/access/coverage kind, and the exact attempted queries or roots.
