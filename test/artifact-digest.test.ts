...
test('public evidence hashing supplies exact local fingerprints without shell access or private authority reads', t => {
  ...
  for (const name of ['activation', 'Activation', 'ACTIVATION']) {
    mkdirSync(join(cwd, '.omd', name), { recursive: true });
    writeFileSync(join(cwd, '.omd', name, 'project.key'), 'synthetic QA data, not a credential');
  }
  ...
  for (const path of ['../escape', '.omd/../escape', '.omd/activation/project.key', '.omd/Activation/project.key', '.omd/ACTIVATION/project.key', '.omd/.project-mutation.lock', '.omd/link', '/etc/hosts']) {
