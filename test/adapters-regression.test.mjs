...
test('codex agent toml survives quotes, colons and backslashes', () => {
  const toml = emitCodex({ agents: [NASTY] }).files['agents/omd-eye.toml'];
  const parsed = parseToml(toml);
  assert.equal(parsed.name, 'omd-eye');
  assert.equal(parsed.description, NASTY.description);
  assert.equal(parsed.model, 'gpt-5.5');
  assert.ok(parsed.developer_instructions.includes('Quote test:'));
});

test('claude agent frontmatter survives quotes and colons', () => {
  const md = emitClaude({ agents: [NASTY] }).files['agents/omd-eye.md'];
  const fm = parseYaml(md.split('---')[1]);
...
test('codex carries deny into the prompt, since it cannot carry it into policy', () => {
  const toml = emitCodex({ agents: [NASTY] }).files['agents/omd-eye.toml'];
  const instructions = parseToml(toml).developer_instructions;
  ...
  for (const key of ['tools', 'allowed_tools', 'disallowed_tools', 'permissions']) {
    assert.equal(parseToml(toml)[key], undefined, `codex agent schema has no ${key}; do not invent one`);
  }
});

test('an agent with no deny list gets no denial prose', () => {
  const plain = { ...NASTY, deny: [] };
  const instructions = parseToml(emitCodex({ agents: [plain] }).files['agents/omd-eye.toml']).developer_instructions;
