...
test('ref list keeps archival inspection but labels ineligible design records instead of promoting them', t => {
  ...
  const refs: Array<{ component: string; admission?: { eligible: boolean; code: string }; discoveryAdmission?: { eligible: boolean } }> = JSON.parse(output);
  assert.equal(refs.find(ref => ref.component === 'legacy-benefits')?.admission?.eligible, false);
  assert.equal(refs.find(ref => ref.component === 'gallery')?.admission?.eligible, false);
  assert.equal(refs.find(ref => ref.component === 'gallery')?.discoveryAdmission?.eligible, true);
