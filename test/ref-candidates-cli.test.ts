...
test('omd ref keeps board operations internal and prints a two-candidate chat table', (context) => {
  ...
  saveRef(root, reference); writeFileSync(image, png()); const input = join(root, 'pinterest-fragment.json');
  writeFileSync(input, JSON.stringify({ inputPath: relative(root, image), provenance: { sourcePage: 'https://www.pinterest.com/pin/123', captureRegion: 'blue editorial crop', licenseStatus: 'unknown', rightsNotes: 'Verify rights before publication.', capturedAt: '2026-07-19T00:00:00.000Z' }, transfer: { visualRole: 'atmosphere', principles: ['Use only as a local abstract accent.'] } }));
  ...
  assert.equal(checked.status, 0, checked.stderr); assert.equal(checked.stdout, '[]\n'); assert.equal(candidates.status, 0, candidates.stderr); assert.match(candidates.stdout, /## 후보 \(Candidate\): 계층 우선/); assert.match(candidates.stdout, /## 후보 \(Candidate\): 분위기 우선/); assert.match(candidates.stdout, /capture\.example/); assert.match(candidates.stdout, /www\.pinterest\.com\/pin\/123/); assert.match(candidates.stdout, /blue editorial crop/); assert.equal(selected.status, 0, selected.stderr); assert.deepEqual(Object.keys(JSON.parse(selected.stdout)).sort(), ['assemblySha256', 'boardSha256', 'candidateId', 'schemaVersion']);
  assert.match(candidates.stdout, /로컬 캡쳐 \(Local capture\)/); assert.match(candidates.stdout, /\.omd\/refs\//);
  writeFileSync(image, png(34)); const stale = run(root, 'ref', 'check', '--json'); assert.equal(stale.status, 1); assert.match(stale.stderr, /board hash/);
