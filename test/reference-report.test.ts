...
test('reference report neutralizes active Markdown and directional control carriers', () => {
  ...
  const snapshot: ReferenceReportSnapshot = { attributionSha256: 'b'.repeat(64), rows: [{ status: 'used', sourcePage: unsafe, sourceRegion: unsafe, target: unsafe, borrowedProperties: [unsafe], nonBorrowedProperties: [unsafe], transformation: unsafe, evidence: unsafe }] };
