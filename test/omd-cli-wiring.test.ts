...
test('printed input skeletons carry exactly the keys their validators accept', async () => {
  ...
  assert.equal(INPUT_SKELETONS.length, 38);
  ...
  const { parseDiscoveryBatchInput } = await import('../core/ref/discovery-batch.ts');
  assert.doesNotThrow(() => parseDiscoveryBatchInput(inputSkeleton('reference-discovery-batch').skeleton));
