...
export function computeHarnessRun(input: HarnessV2RunInput) {
  ...
  const lockDigest=digest(input.evidenceLock.entries
    .filter((entry) => ['E1', 'E2', 'E3', 'E4', 'E5', 'E12'].includes(entry.id))
    .map(({ id, path, kind, schemaVersion, sha256 }) => ({ id, path, kind, schemaVersion, sha256 })));
...
function executableIdentity(command:string):string { if(!isAbsolute(command)||resolve(command)!==command) throw new Error('subprocess command must be an absolute path'); const stat=lstatSync(command); if(!stat.isFile()||stat.isSymbolicLink()||realpathSync(command)!==command) throw new Error('subprocess executable must be a regular resolved file'); return createHash('sha256').update(readFileSync(command)).digest('hex'); }
const pinnedInterpreter = (() => { executableIdentity(process.execPath); return process.execPath; })();
const fixedSubprocessEnvironment = Object.freeze({ PATH: '' });
const fixedSubprocessCwd = realpathSync(tmpdir());
function spawnPinned(command:string,args:readonly string[],options:{input?:string;encoding:'utf8';timeout:number;maxBuffer:number}):ReturnType<typeof spawnSync> { executableIdentity(command); return spawnSync(pinnedInterpreter,[command,...args],{...options,cwd:fixedSubprocessCwd,env:fixedSubprocessEnvironment}); }
...
function recordProcessExecution(source: ParentUsageSource, signed: SignedProcess, child: ReturnType<typeof spawnSync>): ProcessExecution { if (child.status !== 0 || child.signal || !Number.isSafeInteger(child.pid) || child.pid! <= 0) throw new Error(`${source.kind} subprocess lacks a successful child process identity`); const execution=Object.freeze({pid:child.pid!,executableSha256:signed.executableSha256,configIdentity:signed.configIdentity,status:child.status,signal:child.signal,processIdentity:digest({executableSha256:signed.executableSha256,configIdentity:signed.configIdentity,pid:child.pid,status:child.status,signal:child.signal})}); processExecutions.set(processExecutionKey(source),execution); return execution; }
