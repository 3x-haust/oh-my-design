...
export function computeHarnessRun(input: HarnessV2RunInput) {
  ...
  validateFrozenDevelopmentCorpus(input.developmentCorpus); validateEvaluatorHoldoutMetadata(input.evaluatorHoldouts); validateBudget(input.budget); const snapshot=input.evidenceSnapshot === undefined ? validateEvidenceLock(input.evidenceRoot,input.evidenceLock) : requireEvidenceLockSnapshot(input.evidenceSnapshot, input.evidenceLock);
  ...
  const report: HarnessV2RunReport=Object.freeze({schemaVersion:'harness-v2-run-report-v4',signedE5Digest:snapshot.lock.entries.find(entry=>entry.id==='E5')!.sha256,signedE13Digest:snapshot.lock.entries.find(entry=>entry.id==='E13')!.sha256,...live});
