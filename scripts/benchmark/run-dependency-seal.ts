...
import { createDependencySnapshot, RECEIPT, sealSnapshotCandidate, writeCandidateReceipt, type SnapshotInput } from './create-dependency-snapshot.ts';
import { readNativePublisherIdentity, publishDirectoryExclusive, deterministicTreeRoot } from './publish-exclusive.ts';
import { encodeFrame, FrameDecoder, SealStateMachine, archiveLayout, createSealClaim, evaluateEligibility, writeLauncherVerdict, type SealClaim, type LauncherVerdict } from './seal-protocol.ts';

const PHASE_TIMEOUT_MS = 30_000;
export interface DependencySealInput extends SnapshotInput { schemaVersion: 'dependency-seal-input-v1'; archiveRoot: string; sourceFinalizationPath: string; sourceFinalizationSha256: string; sourceFinalizationDomain: string; nativePublisherIdentity: string; sealRunId: string; channelNonce: string; }
export interface DependencySealRun { dryRun?: boolean; input: DependencySealInput; }
type Evidence = { code: number; stdout: string; stderr: string; pid: number; pgid: number; events: string[] };
function fail(message: string): never { throw new Error(`dependency seal: ${message}`); }
function hash(value: unknown): value is string { return typeof value === 'string' && /^[0-9a-f]{64}$/.test(value); }
function canonical(path: string): JsonValue { const raw = readFileSync(path, 'utf8'); let value: JsonValue; try { value = JSON.parse(raw) as JsonValue; } catch { fail(`invalid JSON: ${path}`); } if (canonicalJson(value) !== raw) fail(`non-canonical JSON: ${path}`); return value; }
function exactInput(input: DependencySealInput): void { const keys=['archiveRoot','browserCache','browserDescriptor','browserRevision','channelNonce','nativePublisherIdentity','nodeModules','packageJson','packageLock','schemaVersion','sealRunId','snapshotId','snapshotRoot','sourceFinalizationDomain','sourceFinalizationPath','sourceFinalizationSha256','sourceRoot']; const actual=Object.keys(input).sort(); if (actual.length!==keys.length || actual.some((x,i)=>x!==keys[i])) fail('input has unknown or missing fields'); if(input.schemaVersion!=='dependency-seal-input-v1'||!/^snapshot-\d{3,}$/.test(input.snapshotId)||input.browserDescriptor!=='chromium-headless-shell'||input.browserRevision!=='1228'||!input.sealRunId||!hash(input.channelNonce)||!hash(input.sourceFinalizationSha256)||!hash(input.sourceFinalizationDomain)) fail('input is not canonical seal input'); }
function sourceFinalization(input: DependencySealInput): void { const value=canonical(input.sourceFinalizationPath) as unknown as Record<string,unknown>; if(sha256(canonicalJson(value as JsonValue))!==input.sourceFinalizationSha256||value.schemaVersion!=='source-finalization-v1'||value.status!=='ACCEPTED'||value.domainSha256!==input.sourceFinalizationDomain) fail('source finalization is not the accepted bound record'); }
function durable(path:string,value:JsonValue):void { mkdirSync(resolve(path,'..'),{recursive:true}); const fd=openSync(path,constants.O_WRONLY|constants.O_CREAT|constants.O_EXCL|constants.O_NOFOLLOW,0o600); try { writeFileSync(fd,canonicalJson(value)); fsyncSync(fd); } finally { closeSync(fd); } }
function evidenceRoot(e:Evidence):string { return sha256(canonicalJson({argv:[process.execPath,process.argv[1] ?? ''],code:e.code,events:e.events,pgid:e.pgid,pid:e.pid,stderr:e.stderr,stdout:e.stdout})); }
function workerInput(path:string):DependencySealInput { return JSON.parse(readFileSync(path,'utf8')) as DependencySealInput; }
...
export async function runDependencySeal(run:DependencySealRun):Promise<{dryRun:boolean;inputSha256:string;eligible:boolean}> { const input=run.input; exactInput(input); sourceFinalization(input); readNativePublisherIdentity(input.nativePublisherIdentity); const inputSha256=sha256(canonicalJson(input as unknown as JsonValue)); if(run.dryRun!==false)return {dryRun:true,inputSha256,eligible:false}; const layout=archiveLayout(input.archiveRoot,input.snapshotId),nonce=sha256(`${input.sealRunId}\0${inputSha256}`); const claim:SealClaim={schemaVersion:'dependency-seal-claim-v1',snapshotId:input.snapshotId,path:'claim.json',finalizationPath:input.sourceFinalizationPath,finalizationSha256:input.sourceFinalizationSha256,finalizationDomain:input.sourceFinalizationDomain,sealRunId:input.sealRunId,launcherInputSha256:inputSha256,claimantRecord:input as unknown as JsonValue,claimantRecordSha256:inputSha256,claimantNonce:nonce}; if(!createSealClaim(input.archiveRoot,claim).created)return {dryRun:false,inputSha256,eligible:false}; const inputPath=join(input.archiveRoot,`.${input.snapshotId}.launcher-input.json`); durable(inputPath,input as unknown as JsonValue); let build:Evidence={code:-1,stdout:'',stderr:'',pid:0,pgid:0,events:[]},commit:Evidence={code:-1,stdout:'',stderr:'',pid:0,pgid:0,events:[]},reason=''; let accepted=false, pendingSha='0'.repeat(64), final={tree:'0'.repeat(64),dev:0,ino:0,report:''}; try { build=await phase(['--build-worker','--input',inputPath,'--claim',layout.claim],input); if(build.code!==0) fail('build worker rejected launcher decision'); const candidate=(JSON.parse(build.stdout) as {candidate:string}).candidate; const permit=join(input.archiveRoot,`.${input.snapshotId}.commit.permit`); durable(permit,{schemaVersion:'dependency-seal-permit-v1',snapshotId:input.snapshotId,claimSha256:sha256(canonicalJson(claim as unknown as JsonValue))}); commit=await phase(['--commit-worker','--input',inputPath,'--candidate',candidate,'--permit',permit],input); if(commit.code!==0) fail('commit worker rejected launcher decision'); const receipt=canonical(join(resolve(input.snapshotRoot,input.snapshotId),RECEIPT)); pendingSha=sha256(canonicalJson(receipt)); durable(layout.snapshot,receipt); final=readback(resolve(input.snapshotRoot,input.snapshotId)); accepted=true; reason='published after live FD3 closure'; } catch(error) { reason=(error as Error).message; }
 const verdict:LauncherVerdict={schemaVersion:'dependency-seal-launch-v1.receipt-v1',status:accepted?'ACCEPTED':'REJECTED',snapshotId:input.snapshotId,claimPath:'claim.json',claimSha256:sha256(canonicalJson(claim as unknown as JsonValue)),claimantNonce:nonce,pendingReceiptSha256:pendingSha,permit:`../.${input.snapshotId}.commit.permit`,buildEvidenceRoot:evidenceRoot(build),commitEvidenceRoot:evidenceRoot(commit),observedScope:{directChild:true,pgid:Math.max(build.pgid,1)},finalization:{path:input.sourceFinalizationPath,device:final.dev,inode:final.ino,modes:['read-only'],treeSha256:final.tree,readbackSha256:sha256(final.report),toolIdentities:[input.nativePublisherIdentity]},reason}; writeLauncherVerdict(input.archiveRoot,verdict,input as unknown as JsonValue); return {dryRun:false,inputSha256,eligible:evaluateEligibility(input.archiveRoot,input.snapshotId).eligible}; }
function arg(name:string):string { const i=process.argv.indexOf(name); if(i<0||!process.argv[i+1])throw new Error(`missing ${name}`);return process.argv[i+1]!; }
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href)(async()=>{try{if(process.argv.includes('--build-worker'))await buildWorker(arg('--input'),arg('--claim'));else if(process.argv.includes('--commit-worker'))await commitWorker(arg('--input'),arg('--candidate'),arg('--permit'));else console.log(canonicalJson(await runDependencySeal({input:JSON.parse(readFileSync(resolve(arg('--input')),'utf8')) as DependencySealInput,dryRun:!process.argv.includes('--publish')}) as unknown as JsonValue));}catch(error){console.error((error as Error).message);process.exitCode=1;}})();
