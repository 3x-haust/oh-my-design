...
- Codex CLI 지원은 트리에 있고 경로 해석이 이제 호스트 중립적이다: 에이전트와 스킬이 `omd pack dir`을 통해 지식 팩을 찾기 때문에 Claude Code 전용 env var에 의존하지 않으며, 회귀 테스트는 두 호스트를 모두 검증한다. 검증되지 않은 부분은 실제 설치 흐름 — 마켓플레이스가 실제 세션에서 plugin.json, skills/, agents/를 로드하는 방식 — 이다.
