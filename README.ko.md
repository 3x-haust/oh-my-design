...
```text
브리프 → 근거 → 카피 → 격리된 구조안 → 프로덕션 구현 1회
       → 렌더 비평과 인터랙션 증거 → 문제 재정의
```

OMD는 Codex와 Claude Code에서 동작합니다. 사용자용 스킬 6개, 내부 파이프라인 에이전트 7개, 로컬 CLI, 디자인 이론·레시피 팩, `.omd/` 프로젝트 기록으로 구성됩니다.
...
7. **구조 발산**: 기본은 서로 격리된 구조안 2개입니다. 쇼피스이거나 구조 불확실성과 영향이 크면 3개를 만듭니다.
8. **블라인드 선택**: 새 선택 담당자가 익명 후보를 과업 적합성, 실제 콘텐츠 수용력, 위계, 반응형 비용, 접근성 위험으로 비교합니다.
9. **한 번 구현**: 선택한 구조 하나만 프로덕션 코드로 만듭니다. 구현 담당자는 후보를 다시 늘리지 않습니다.
10. **만들면서 성찰**: 실제 콘텐츠로 의미 구조를 잡은 뒤 semantic checkpoint를 남기고, 시각 체계를 적용한 뒤 visual checkpoint를 남깁니다. 모션은 그 다음입니다.
11. **결과 관찰**: 데스크톱·모바일 렌더, squint 렌더, 필요한 filmstrip, 규칙 검사, 선언된 로컬 probe가 리뷰 증거가 됩니다.
12. **비평과 재정의**: squint만 보는 glance가 위계를 먼저 적습니다. 별도의 고해상도 렌더 리뷰어가 완성도와 사용자 결과를 판단하고, 진행 담당자가 렌더를 보고 달라진 문제 정의를 기록합니다.
13. **인계**: 프로젝트 테스트와 빌드, 적용 가능한 디자인 게이트, 남은 검사 결과와 근거를 함께 보고합니다.
...
| 구조 스케치 | `.omd/.cache/sketches/<id>/` | 각 후보는 정제된 frame, 실제 카피, 구조 축 하나, 익명 ID만 받습니다. 다른 후보나 프로덕션 소스를 볼 수 없습니다. |
...
아래 7개는 사용자가 호출하는 공개 명령이 아니라 디자인 루프 내부 역할입니다.
...
| `omd-eye` | 익명 구조를 고르거나, 카피를 블라인드 리뷰하거나, sharp 렌더를 비평합니다. | 쓰지 않습니다. |
